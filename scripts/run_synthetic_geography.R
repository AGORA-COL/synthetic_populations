...
schools_list = read_delim(schools_file, delim = ";", locale = locale(encoding = "latin1"), trim_ws = T) %>%
    ...
    mutate(school_id =  sprintf('%03d%06d%07d', country_code,city_code,row_number()),
           name = gsub(",","",name))
...
universities_df = universities_df %>%
    dplyr::mutate(school_id =  sprintf('%03d%06d%07d', country_code,city_code,row_number() + nrow(school_df)),
...
workplace_df = workplace_df %>%
    mutate(mun_code = city_code,
           workplace_id =  sprintf('%03d%06d%07d', country_code,city_code,row_number()))
