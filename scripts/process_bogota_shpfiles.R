...
esc_data <- esc_shp %>%
  dplyr::select(SCACODIGO, SCANOMBRE) %>%
  dplyr::mutate(Localidad = 0)

#esc_coor = coordinates(esc_shp)
#colnames(esc_coor) = c("LAT", "LON")
#esc_coor = as.data.frame(esc_coor)
esc_coor = st_coordinates(esc_shp)[,1:2]
colnames(esc_coor) = c("LAT", "LON")
esc_coor = as.data.frame(esc_coor)
#coordinates(esc_coor) = ~ LAT + LON
#proj4string(esc_coor) = proj4string(localities_shp)
esc_coor_sf <- st_as_sf(esc_coor, coords = c("LON", "LAT"), crs = st_crs(localities_shp))
