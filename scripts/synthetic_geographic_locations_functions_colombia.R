...
lonlat2UTM <- function(lonlat) {
  # Ensure the input is a valid lon/lat pair
  if (length(lonlat) != 2 || lonlat[1] < -180 || lonlat[1] > 180 || lonlat[2] < -90 || lonlat[2] > 90) {
    stop("Invalid longitude or latitude values provided.")
  }
  
  # Calculate UTM zone number
  utm_zone <- (floor((lonlat[1] + 180) / 6) %% 60) + 1
...
download_worldpop_data = function(data_dir_dest = "../data/raw_data/geodata",
    ...
    metadata_worldpop = RCurl::getURL(sprintf("https://hub.worldpop.org/rest/data/pop/pic?iso3=%s",country_name_gdam))
...
#' synthesize_locations_zones_mun
#'
#' The purpose of this function is to geo-locate houses and other locations using openstreetmaps and worldpop as a starting point
#' 
#' @param house_file (dataframe) synthesized houses dataframe (from synthesize_population())
...
#' synthesize_locations_zones()
#'
synthesize_locations_zones_mun = function(house_file, 
                                country_name,
                                country_name_gdam,
                                country_code,
                                departamento_name,
    ...
    ## We're looking for municipalities    
    city_indx = country_data_shp %>%
        filter(as.numeric(COD_DEPTO) == departamento_code) %>%
        pull(ROW)
    ...
    country_raster_file = download_worldpop_data(data_dir_dest = raw_geodata_path,
                                                    country_name = country_name,
                                                    country_name_gdam = country_name_gdam)
    if(length(country_raster_file) == 0){
        stop("country_raster_file invalid, something must've gone wrong with the worldpop download manager")
    ...
    print(sprintf("cropping raster with shapefile of department: %s", departament_name))
    print(proj4string(country_raster))
    print(sprintf("minx: %.2f maxx: %.2f miny: %.2f maxy: %.2f\n",
                    city_bbx[1,1], city_bbx[1,2],city_bbx[2,1],city_bbx[2,2]))
    mid_x = city_bbx[1,1] + (city_bbx[1,2] - city_bbx[1,1]) / 2
    ...
    ## Now, I have to convert the population density to a probability
    ## Then, I have to draw a cell, based on that probability
    print(sprintf("creating probability array from population density grids for %s", departament_name))

    ##ZONE =  tolower(iconv(Nombre_de_l,from="UTF-8",to="ASCII//TRANSLIT")))
    houses_df$Municipio = 0
    houses_df$LAT = 0
    houses_df$LON = 0

    for(zz in 1:length(departamento_levels)){
        municipio_code <- as.character(departamento_levels[[zz]])
        cat("\rMunicipio: ", municipio_code)

        ## We're looking for municipalities    
        municipio_indx = municipio_data_shp %>%
            filter(as.numeric(ID_ESPACIA) == as.numeric(municipio_code)) %>%
            pull(ROW)

        ## We're looking for municipalities    
        municipio_UE_indx = municipio_UE_data_gdb %>%
            filter(as.numeric(COD_MPIO) == as.numeric(municipio_code)) %>%
            pull(ROW)


        municipio_UE_shp_ <- spTransform(municipio_UE_gdb[municipio_UE_indx, ], 
                                        CRS("+proj=longlat +datum=WGS84"))

                                        
        municipio_shp_ <- spTransform(municipio_shp[municipio_indx,], 
                                        CRS("+proj=longlat +datum=WGS84"))
        ...
        block_shp = municipio_UE_shp_
        zone_shp = municipio_shp_

        proj4string(zone_shp) = proj4string(city_shp)
        zone_houses_ind = which(houses_df$ZONE == as.numeric(municipio_code))
        if(length(zone_houses_ind) == 0){next}

        houses_df$Municipio[zone_houses_ind] = as.numeric(municipio_code)
        zone_blocks = block_shp#[block_shp@data$SCACODIGO == zones_data_shp$ZONE[zz],]
        if(nrow(zone_blocks) == 0){
            zone_blocks = zone_shp
        }
    ...
    city_houses = as_tibble(houses_df) %>% mutate(landuse = "HOUSE") %>%
        dplyr::mutate(longitude = LON, latitude = LAT, sp_id = HHID,
                        stcotrbg = sprintf("%d%s", departamento_code, ZONE),
                        serialno = SERIAL,hh_race = 1, hh_size = HHSIZE, hh_age = AGE,hh_income = 1) %>%
        dplyr::select(sp_id, serialno, stcotrbg, hh_race, hh_income, hh_size, hh_age, latitude, longitude)
