...
#'
#' 
assign_schools <- function(people_df, houses_df, schools_df, priv_schools_df,
                           students_mobility_df,
                           geodata_info,esc_shp){
    ...
    for(ll in 1:length(localidad_list)){
        ...
        for(gg in 1:length(grades_list) ){
            ...
            print(sprintf("Grade %s for Public schools locality %d. Size of schools %d, size of students %d", grades_list[gg], tmp_loc, nrow(tmp_schools), length(tmp_ind_students)))
        ...
        for(gg in 1:length(grades_list) ){
            ...
            print(sprintf("Grade %s for Private schools locality %d. %d schools with %d students", grades_list[gg], tmp_loc, nrow(tmp_schools), length(tmp_ind_students)))
...
#'
#' 
assign_colleges <- function(people_df, houses_df,
                            college_df,
                            geodata_info,esc_shp){    
...
#'
#' 
assign_workplaces <- function(people_df, houses_df, 
                                      workers_mobility,
                                      workplaces_df, geodata_info,esc_shp){
    geodata_info = geodata_info %>% drop_na() %>% dplyr::select(SCACODIGO, Localidad) %>%
        group_by(SCACODIGO) %>% summarize(Localidad = Localidad[1])
    localidad_list = sort(unique(geodata_info$Localidad))
